...
export const redirectGit = async (req, res) => {
    try {
        ...
        const { data } = await axios.post(`https://api.github.com/app-manifests/${code}/conversions`, {}, {
            headers: {
...
export const getGitApps = async (req, res) => {
    try {
        const sessionEmail = req.session.user?.email;
        if (!sessionEmail) {
            return res.status(401).json({
...
export const deleteGitApp = async (req, res) => {
    try {
        const sessionEmail = req.session.user?.email;
        if (!sessionEmail) {
            return res.status(401).json({
...
export const getGitRepos = async (req, res) => {
    try {
        const sessionEmail = req.session.user?.email;
        if (!sessionEmail) {
            return res.status(401).json({
...
export const regenerateGitAppToken = async (req, res) => {
    try {
        const sessionEmail = req.session.user?.email;
        if (!sessionEmail) {
            return res.status(401).json({
...
export const updateGitUrl = async (req, res) => {
    try {
        const sessionEmail = req.session.user?.email;
        if (!sessionEmail) {
            return res.status(401).json({
