...
export const createInstance = async (req, res) => {
    try {
        const { name, type, gitUrl, volume, ports, domains, enviorement } = req.body;
        const slug = String(req.params.slug);
        if (!name) {
            return res
                .status(400)
...
// Server actions
export const syncComposeFile = async (req, res) => {
    try {
        const slug = String(req.params.slug);
        await composeFileSync(slug);
        res
...
export const deployInstance = async (req, res) => {
    try {
...
export const logsInstance = async (req, res) => {
    try {
        const { islug, slug } = req.params;
        const instance = await prisma.instance.findFirst({
            where: {
